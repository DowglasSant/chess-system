...
public class Pawn extends ChessPiece {
	...
	public boolean[][] possibleMoves() {
		...
		if(getColor() == Color.WHITE) {
			...
			auxiliarPosition.setValues(position.getRow() - 2, position.getColumn());
			Position p2 = new Position(position.getRow() -1, position.getColumn());
			if(getBoard().positionExists(auxiliarPosition) && !getBoard().thereIsAPiece(auxiliarPosition) && getBoard().positionExists(p2) && !getBoard().thereIsAPiece(p2) && getMoveCount() == 0) {
				mat[auxiliarPosition.getRow()][auxiliarPosition.getColumn()] = true;
			}
